...
def _load_keras_layer(hdf5_file, layer_name):
  ...
  """
    return _load_keras_weights(hdf5_file=hdf5_file, layer_name=layer_name), _load_keras_biases(hdf5_file=hdf5_file,
...
class BestWeightsTracker:
    ...
    def save_best_weights(self, model):
        if self._best_state is not None:
            ...
            print("Saved best model weights (Mean Average Precision = %1.2f%%) to '%s'" % (
            self._best_mAP, self._filepath))